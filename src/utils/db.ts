...
const searchTasks = async(searchTerm:string)=>{
    const resTitle = await databases.listDocuments(dbID,collectionID,[Query.search('title',searchTerm)]);
    const resDesc = await databases.listDocuments(dbID,collectionID,[Query.search('description',searchTerm)]);
...
export {createDocument,readDocument,updateDocument,deleteDocument,searchTasks,sortByDueDate}